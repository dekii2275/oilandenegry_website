...
export const useProductDetail = (productId: string) => {
  ...
  // ✅ baseUrl: ưu tiên env, fallback /api để chạy qua nginx
  const baseUrl = process.env.NEXT_PUBLIC_API_URL?.trim() || "/api";

  const pickStoreName = (data: any): string => {
    return (
      data?.store_name ||
  ...
  // ============================================================================
  // 1. FETCH PRODUCT DETAIL (Dùng Fetch thuần)
  // ============================================================================
  useEffect(() => {
      ...
      try {
        ...
        const categoryName =
          typeof data.category === "string"
            ? data.category
            : data.category?.name || "Công nghiệp";

        const productFromApi: ProductDetail = {
          id: data.id,
          name: data.name,

          // ✅ ĐỔI: ưu tiên store_name -> fallback brand -> fallback cố định (không còn No Brand)
          brand: storeName || data.brand || "ZEnergy Store",
          ...
          technicalDetails: {
            brand: storeName || data.brand || "Đang cập nhật",
            model: data.variants?.[0]?.sku || data.sku || "N/A",
            warranty: data.warranty || "12 tháng",
            origin: data.origin || "Việt Nam",
          },

          supplier: {
            id: data.store?.id || data.store_id || 1,
            // ✅ dùng store_name luôn
            name: storeName || "ZEnergy Store",
            logo: "/images/default-store.png",
            slug: "z-energy-store",
          },

          specifications: data.specifications || {},
          features: ["Bảo hành chính hãng", "Giao hàng toàn quốc", "Hỗ trợ kỹ thuật 24/7"],
          sku: data.variants?.[0]?.sku || data.sku || "N/A",
  ...
  const checkAuthAndRedirect = (
    actionType: "buy-now" | "add-to-cart" | "wishlist"
  ) => {
  ...
  const handleRequestQuote = async () => {
    ...
    try {
      ...
      else toast.error("Lỗi hệ thống!", { duration: 4000, icon: "❌" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmitReview = () => {
    toast.success("Cảm ơn đánh giá của bạn!", { duration: 2500 });
